...
with a class variable. In javascript we achieve this through closures and immediately-invoked function 
expressions.
...
But that's not all. Cats can change weight. Use Object.defineProperty to write custom 
setters and getters for the weight property so that the following works properly even as
 instances change their weight value:

felix.weight = 25;
felix.weight // 25
Cat.averageWeight(); // now 25
Object.defineProperty must be used to pass all tests. Storing a reference to all instances and 
recalculating the average weight each time is easier, but would prevent garbage collection from working properly if used in a production environment.

Finally, since average weight is an aggregate statistic it's important that we validate constructor 
arguments so that no cats are created without a specified weight; 
so, make sure to throw an error if both arguments are not recieved by the constructor.

*/
const Cat = (function (name, weight) {
  const cats = []
  const getAndSet = function (name, weight) {
    if (!name || !weight)
  ...
  getAndSet.averageWeight = () => cats.reduce((sum, cat) => (sum + cat.weight), 0) / cats.length;
  return getAndSet;
}());

const fluffy = new Cat("fluffy", 10);
const greepy = new Cat("greepy", 20);
console.log(fluffy.name); // fluffy
console.log(Cat.averageWeight()); // 15
